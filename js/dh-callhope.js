/* ══════════════════════════════════════════════════════════════════════
   ...
   ⚠ 구글 쪽(당현함_신청폼.gs)이 요청사항을 200자에서 자릅니다.
     그래서 시간을 고르면 손님이 쓸 수 있는 글자 수를 [통화] 줄만큼 줄여 보여 줍니다.
   ══════════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  window.dh통화붙이기 = function (memo) {
    ...
    if (!줄) return memo;
    if (!memo) return 줄;
    /* 제품 줄(dh-pick.js)이 붙어 있으면 그대로 두고, 손님 글만 있으면 [요청] 을 달아 줄을 가른다 */
    var 합친 = /^\[(선택|요청)\]/.test(memo) ? 줄 + '\n' + memo : 줄 + '\n[요청] ' + memo;
    return 합친.length > 200 ? 합친.slice(0, 200) : 합친;
  };
