...
(function () {
  ...
  var 번호 = '1600-4670';
  /* 상담 시간 — 화면에 보여 주는 글. 위 「구간」 표와 같은 시간입니다(2026-09-23) */
  var 시간글 = '평일 9:00–18:00 · 토 10:00–14:00';
  ...
  window.dh통화칸넣기 = function () {
    ...
    고름.addEventListener('change', function () {
      if (고름.value === 'write') {
        ...
        var 글칸 = document.getElementById('inputMemo');
        /* 적기 전에 상담 시간을 보여 줍니다 — 쉬는 때를 적는 일을 줄입니다 (2026-09-23) */
        if (글칸) { if (!글칸.value) 글칸.placeholder = '원하시는 통화 시간을 적어주세요 (' + 시간글 + ')'; 글칸.focus(); }
      }
      글자한도맞추기();
    });
    var 글칸 = document.getElementById('inputMemo');
  ...
  function 안내줄(고름, 직접글있음, 번호글) {
    ...
    if (고름 && 고름.value === 'write' && 직접글있음) {
      /* 2026-09-23 — 전에는 「남기신 시간에 전화드립니다」였습니다. 손님이 일요일처럼 쉬는 때를 적어도
         그 시간에 걸겠다고 약속하는 셈이었습니다(적은 글의 뜻은 사이트가 알 수 없습니다).
         「확인해서 상담 시간 안에」로 바꾸고 상담 시간을 함께 보여 줍니다(사장님 「나」안). */
      return '남기신 시간을 확인해<br>상담 시간 안에 ' + 번호글 + ' 번호로<br>전화드립니다.' +
             '<span class="dh-call-done" style="display:block;margin-top:6px;">상담 시간 · ' + 시간글 + '</span>';
    }
