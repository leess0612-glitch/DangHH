/* ===========================================================
   ...
   ⚠ 이 파일을 고치면 이 파일을 부르는 모든 화면의 ?v= 숫자를 같이 올려야 한다.
     ...
     지금 부르는 곳: 7곳. 찾는 법(DangHH 폴더에서):  grep -l "dh-apply.js" *.html
...
(function () {
  ...
  (function () {
    ...
    s.id = 'dhCallhopeJs';
    s.src = 나.replace(/[^/]*$/, 'dh-callhope.js?v=2');
  ...
  function 모양심기() {
    ...
    st.textContent = [
      ...
      '#applyOverlay .form-headline span{color:var(--primary,#1257C9);}',
      ...
      '#applyOverlay .service-tab.active{border-color:var(--primary,#1257C9);background:#eff6ff;color:var(--primary,#1257C9);}',
      ...
      'background:#fffbeb;border:1px solid #fde68a;border-radius:8px;margin-bottom:16px;cursor:pointer;}',
        ...
        '<div class="hero-form-wrap" id="applyForm">' +
          '<div class="form-card">' +
            '<div id="formContent">' +
              ...
              '<p style="margin-top:10px;font-size:13px;color:var(--gray,#64748b);text-align:center;">타업체가 더 준다면? <strong style="color:var(--primary,#1257C9);">차액 120% 보상</strong></p>' +
            '</div>' +
