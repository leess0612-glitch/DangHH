      ...
      window.submitForm = function () {
        ...
        if (칸 && 고른.length) {
          /* ⚠ 손님이 직접 쓴 글은 200자까지입니다. 제품 줄은 그 위에서 따로 붙이므로
             합친 글이 200자를 넘을 수 있습니다. maxlength 는 사람이 칠 때만 막습니다.
             ⚠(2026-09-21 바로잡음) 시트 한 칸이 5만 자까지 든다고 적어 두었는데, 실제로는
             구글 쪽(당현함_신청폼.gs 메모다듬기_)이 **200자에서 뒤를 자릅니다.** 제품 줄이 길면
             맨 아래 손님 글 [요청] 끝이 잘릴 수 있습니다. 제품 줄과 [통화] 줄은 위에 있어 남습니다.
             ★(2026-09-21) 보낸 뒤 칸을 손님이 쓴 글로 되돌립니다. 전에는 합친 글이 칸에 남아,
             실패해서 다시 누르면 제품 줄이 두 번 붙었습니다([요청] [선택]…). 신청창은 글을
             누른 순간에 읽어 가므로 곧바로 되돌려도 보내는 글은 그대로입니다. */
          var 손님원문 = 칸.value;
          칸.value = 합친글(칸.value.trim());
          try { return 옛보내기.apply(this, arguments); }
          finally { 칸.value = 손님원문; }
        }
