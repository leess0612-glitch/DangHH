...
(function () {
  ...
  var 보임 = 처음보임;
  var 고른갈래 = '전체';
  /* 코웨이 전용 화면(/water-c/)은 **브랜드가 코웨이로 골라진 채** 열립니다 (2026-09-18 사장님 지시).
     코웨이 광고로 오신 손님이 코웨이부터 보시게 하려는 것입니다.
     다른 브랜드를 감추지는 않습니다 — 알약에서 바꾸면 그대로 다 보입니다. */
  var 코웨이칸 = /\/water-c(\/|$)/.test(location.pathname);
  var 고른거름 = { 브랜드: 코웨이칸 ? '코웨이' : '전체', 형태: '전체', 정수: '전체', 관리: '전체' };
  var 고른정렬 = '인기순';
  var 모두 = [];
  ...
  /* 2026-09-22 사장님 지시 — 주요 브랜드는 코웨이·SK매직·쿠쿠·LG 넷입니다.
     청호나이스·교원웰스·루헨스·현대큐밍은 인기순에서 이 넷보다 뒤에 섭니다. */
  var 주요브랜드 = ['coway', 'sk', 'cuckoo', 'lg'];
  function 브랜드차례(코드) {
  ...
  function 모으기() {
    ...
    (window.브랜드모음 || []).forEach(function (자료) {
      ...
      (자료.제품 || []).forEach(function (p, i) {
        ...
        p.명단 = 자리 >= 0;                                        /* 인기 명단에 적힌 제품인가 */
        p.브랜드급 = 주요브랜드.indexOf(자료.코드) >= 0 ? 0 : 1;   /* 0 = 주요 브랜드 */
        p.앞자리 = 9999;
  ...
  function 줄세우기(목) {
    ...
    if (고른정렬 === '낮은 요금순') {
      ...
      /* 인기순 — ①사장님이 정한 앞줄이 맨 먼저
                  ②그 뒤는 인기 명단에 적힌 제품 → 그중 주요 브랜드(코웨이·SK매직·쿠쿠·LG) 먼저
                    (2026-09-22 사장님 지시 — 전에는 브랜드를 번갈아 섞어 청호·루헨스·큐밍·웰스가 13번째부터 몰려 나왔습니다)
                  ③같은 무리 안에서는 브랜드마다 정한 자리를 번갈아 섞어 한 브랜드가 몰리지 않게
                  ④자리가 같으면 브랜드 차례(코웨이 → SK매직 → …)로 가릅니다
         브랜드 하나만 골랐을 때는 ②의 브랜드 구분이 없으므로 차례가 전과 똑같습니다. */
      ㄱ.sort(function (a, b) {
        if (a.앞자리 !== b.앞자리) return a.앞자리 - b.앞자리;
        if (a.명단 !== b.명단) return a.명단 ? -1 : 1;
        if (a.브랜드급 !== b.브랜드급) return a.브랜드급 - b.브랜드급;
  ...
  /* ── 기능 그림 (2026-09-17 사장님 결정: 대안 다섯 중 ②「색 그림 나열」) ──────────────
     ...
     기능은 얼음·냉수·온수·정수 네 가지의 조합이라, 든 것만 색 그림으로 늘어놓습니다.
       얼음 = 하늘색 얼음 조각 · 냉수 = 파란 물방울 · 온수 = 빨간 물방울 · 정수 = 초록 물방울(흰 반짝임)
  ...
  var 조각 = {
    ...
    /* 온수도 물방울입니다 (2026-09-18 사장님 지시 — 불꽃이 혼자 딴 모양이었습니다).
       모양은 냉수와 같고 색만 다릅니다. */
    온: function (c) { return '<path d="M12 3c-3.5 4-6 7-6 10a6 6 0 0012 0c0-3-2.5-6-6-10z" fill="' + c + '"/>'; },
